...
if 'REUSE' not in os.environ:
    ...
    print('Syncing code ...')
    sync_code = subprocess.Popen(
        f"rsync -az --exclude-from .gitignore --exclude '.git/' . {host_name}:{directory}",
        shell=True)

    print('Syncing git ...')
    sync_git = subprocess.Popen(
        f"rsync -az --delete .git {host_name}:{code_path}/{proj_dir}",
        shell=True)
...
if chain_count > 1:
    for i in range(chain_count):
        if i > 0:
            dependency = '--dependency=afterok:"$id"'
